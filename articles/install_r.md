...
same warnings and errors as the
screenshot.](install_r_images/R_screenshot.png)
...
![Example of recently-opened Rstudio. Note that the appearance of
RStudio is customizable, so your interface might not look the same as
the RStudio your instructors or your fellow students
have](install_r_images/RStudio_screenshot.png)
...
![](install_r_images/set_dir_fig00.png)
...
![](install_r_images/set_dir_fig0.png)
...
![](install_r_images/set_dir_fig1.png)
...
![](install_r_images/set_dir_fig2.png)
...
**(I)** To check what working directory you are in, type
[`getwd()`](https://rdrr.io/r/base/getwd.html) into the console or into
a script file. This function will tell you which folder is currently set
as the working directory by giving you a path name to the folder. If you
decided to store your folder in the Desktop, you should see something in
the following format (note this follows Mac/Linux formatting, Windows
will write this slightly differently):
...
working directory is set, run
[`getwd()`](https://rdrr.io/r/base/getwd.html) and paste its output into
the [`setwd()`](https://rdrr.io/r/base/getwd.html) function. Insert that
line in your “setup” code chunk. This will help RMarkdown to compile
more smoothly. It will look more or less like the following screenshot:

![](install_r_images/set_dir_fig3.png)
...
[`dir()`](https://rdrr.io/r/base/list.files.html) into the console or
into a script file. This will list the names of every file or subfolder
within the working directory. For example, if I had two files in my
working directory named `cyt-oxidase1-mt-aligned.txt` and
`R_functions.R`, I would see the following output after typing
...
[`ls()`](https://rdrr.io/r/base/ls.html), which gives you a list of all
objects currently loaded in your R environment. Before you start your
lab, the results should always be `character(0)`, which means there is
...
**IF** your result from [`getwd()`](https://rdrr.io/r/base/getwd.html)
is different from what it should be, try step 1 (setting your work
directory) again.

**IF** your result from
[`dir()`](https://rdrr.io/r/base/list.files.html) is different from what
it should be, you could either be in the wrong folder (so go back to
step 1) or you could not have all the files in the correct folder –
correct this by moving files in your computer.
...
![](install_r_images/set_dir_fig5.png)
...
Now, if you run [`dir()`](https://rdrr.io/r/base/list.files.html) again,
you should see your saved file. As I saved my file with the name
`lab_05_answers.Rmd`, I got:
...
![](install_r_images/set_dir_fig6.png)

Now, if you run [`dir()`](https://rdrr.io/r/base/list.files.html) again,
you should see your new compiled `.html` document in the working folder:
...
[MJ’s github](https://mjanuario.github.io/evolved/articles/) so that you
can see how each of those two files communicate (and how Markdown works
“under the hood”).
...
(posted on [MJ’s github](https://mjanuario.github.io/evolved/articles/)
to learn the syntax associated with everything we are doing below.
...
$N_{e} = \frac{1}{\frac{1}{t}\sum_{i = 1}^{t}\frac{1}{N_{i}}}$
...
$\frac{H_{t}}{H_{0}} = \left( e^{- \frac{1}{2N}} \right)^{t}$
...
![](install_r_files/figure-html/unnamed-chunk-13-1.png)

#### RMarkdown practice (part I)

Please take a look of the RMarkdown used to make this file (in [MJ’s
github](https://mjanuario.github.io/evolved/articles/) and try to locate
the code responsible for each part of what you have just read, putting
special attention to codes, plots, and equations.
