...
#' @plot Logical indicating if simulations should be plotted as colored 
#' lines. Each color represents a different population. Default value 
#' is \code{TRUE}.
#' @printData Logical indicating weather all simulation resutls should be 
...
#' with each line+color refering to a different simulation. Note that if 
#' may simulations (generally more than 20) are simulated, colors might be 
#' cycled and different simulation will have the smae color. If 
...
#' WFDriftSim(N = 5, nGens = 30, p=1)
#' 
#' #Many populations::
#' WFDriftSim(N = 5, nGens = 30, p0=.2, nSim=20)
#' 
...
WFDriftSim=function(N, nGens, p0=0.5, nSim=1, plot=TRUE, printData=FALSE){
  ...
  for(generation in 1:nGens){
    p_through_time <- cbind(p_through_time, 
          rbinom(n = nSim, size = n_alleles, prob = p_through_time[,generation]) / n_alleles
          )
  }
  ...
  if(plot){
    ...
    cols=rainbow(nSim)
    for(i in 1:nSim){
      lines(x = 0:nGens, y = p_through_time[i,], col=cols[i])  
    }
  }
