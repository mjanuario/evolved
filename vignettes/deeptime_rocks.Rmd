...
We've downloaded 4 fossil datasets for you to work with today, including mammals (`mammals_fossil`), dinosaurs (`dinos_fossil`), ammonites (`ammonoidea_fossil`), and trilobites (`trilob_fossil`). All occurrence data were originally obtained from the Paleobiology database, a free-to-use resource that is the standard repository for such data and which has been used by hundreds or thousands of researchers to study the history and dynamics of biodiversity in deep time. 
...
**`early_interval`**, **`late_interval`** = The name of the early (furthest from present) and the name of the late (closest to present) intervals associated with that occurrence. Some of those intervals appear in the [GSA geologic timescale (link **here**)](https://www.geosociety.org/GSA/Education_Careers/Geologic_Time_Scale/GSA/timescale/home.aspx). 
...
```{r}
plotRawFossilOccs(Trex, use.midpoint = F)
...
\begin{figure}[H]
  \includegraphics{~/Dropbox (University of Michigan)/devel/2024-labs/L06/example_fossil_occs_V2.png}
...
**The standard method**, which simply sums up taxa that co-occur in the same interval.

**The range-through method**, which only sums up species that are observed before and after a given stratigraphic interval. This method guarantees we are measuring diversity that co-occurs in time. 
...
knitr::include_graphics('~/Dropbox (University of Michigan)/devel/2024-labs/L06/zircon_dating.png')
```
...
As an extra reading, you can find an interesting outreach article about zircon dating [ here](https://knowablemagazine.org/article/physical-world/2021/keeping-time-zircons).
