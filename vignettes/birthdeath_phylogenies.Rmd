...
To get a sense for this, we will simulate a phylogeny with very high relative extinction ($E / S = 0.99$), and a pure-birth tree ($E / S = 0$) for comparison. Note: it doesn't make much sense to specify $E / S >= 1$ because this evolutionary process will almost always go extinct before speciating, thus leaving no tree -- so we will omit this case.
...
tree_highE <- simulateTree(pars=c(1, 0.99), max.taxa=100, max.t = Inf)
...
Describe, in a sentence or two, how the overall structure of these two trees appears to differ.
\end{minipage}}

Now we will visualize lineage-through-time (LTT) plots for these 2 trees, just like we did a few weeks ago. The LTT plots contain a lot of information about phylogenetic tree structure. Let's see what this can tell us about the *shape* of trees produced by different speciation and extinction rates.

```{r}
# plot lineage through time plots:
lttPlot(tree_pb, col = "blue")

lttPlot(tree_highE, col = "red")

plot.new()
plot(x=c(0,1), y = c(log(2), log(100)), lwd=1, col="gray50", type = "l")
lttPlot(tree_pb, rel.time=T, add=T, col="blue", lwd=1.5)
lttPlot(tree_highE, rel.time=T, add=T, col="red", lwd=1.5)
```
...
To explore this, let's simulate a set of phylogenies under high relative extinction (E / S = 0.95) and do the same thing we did previously, again estimating speciation under the simple (pure birth) estimator.
...
for(i in 1:REPS){
  ...
  # Now we will change the extinction fraction to always equal 0.95
	sim_E <- sim_S * 0.95
...
State a single-line hypothesis about why these simulations need more time to be completed, in comparison with the pure-birth simulations.

Hint: Look at all the parameters we used when we set up our simulation.
\end{minipage}}

Let's check our estimates for the $E/S = 0.95$ case.
...
Were you still able to estimate the speciation rate using this model? If the estimates are biased, how so?
\end{minipage}}
...
Let's repeat our simulation procedure, but this time we will fit a model where extinction can be nonzero.
...
Now, let's check our estimates for the speciation rate:
...
**Note:** It is not adequate to directly correlate simulated and estimated extinction rates. This happens because we can only generate phylogenies when $E < S$, and thus we do not allow extinction to vary n a way that is completely independent from speciation. However, when we correlate simulated and estimate $K = E/S$, we ameliorate this particular issue enough (for this vignette).
...
```{r}
plot(x=data_whales$log_mass, data_whales$S, pch = 18,
...
legend(x="topright",legend=c("other odontocetes","Baleen whales",
    ...
    pch=21,pt.cex=1.5,pt.bg=c("black",
...
plotPaintedWhales(ftype="off", direction="rightwards", mar=c(5.1,0,2.1,2.1), show.legend = TRUE)
